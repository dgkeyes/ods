...
dk_make_map <- function(esd_name) {

     for (i in 4:1) {

          temp <- filter(ods.esd.map.data, participation == map.filters[i])
     ...
     saveWidget(frameableWidget(ods.map.by.esd),
                paste(str_replace_all(str_to_lower(esd_name), " ", "-"),
                      ".html",
                      sep = ""))
...
esd.list <- unique(schools.data.for.map.v2$esd) %>%
     tibble() %>%
     set_names("ESD") %>%
     filter(ESD != "Private" &
                 ESD != "Oregon Department of Education")


for (j in 1:length(esd.list$ESD)) {

     ods.esd.map.data <- schools.data.for.map.v2 %>%
          filter(esd == esd.list$ESD[j])

     dk_ods_map(esd.list$ESD[j])
